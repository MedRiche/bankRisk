...
def import_german_credit_data(csv_path):
    ...
    print("🔄 Importation des données German Credit...")
    ...
    try:
        ...
        print("💡 Placez le fichier german_credit_data.csv dans le dossier backend/")
        return
    ...
    for index, row in df.iterrows():
        try:
            ...
            application = CreditApplication(
                ...
                purpose=purpose,
                risk=row['Risk'].lower(),
                status='approved' if row['Risk'].lower() == 'good' else 'rejected'
            )
            application.save()
    ...
    print(f"   Risque good: {CreditApplication.objects(risk='good').count()}")
    print(f"   Risque bad: {CreditApplication.objects(risk='bad').count()}")
